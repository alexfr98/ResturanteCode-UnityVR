...
public class OVRGrabber : MonoBehaviour
{
    ...
    public bool isGeneratorIng = false;
    ...
    private GameObject colisioActual = null;
    void OnTriggerEnter(Collider otherCollider)
    {
        ...
        colisioActual = otherCollider.gameObject;
        // Add the grabbable
        int refCount = 0;
    ...
    protected virtual void GrabBegin()
    {
        ...
        if (colisioActual.tag == "GeneratorIng")
        {
            colisioActual = (new FactoryIngredients()).getIngredient(this.colisioActual.gameObject);

            closestGrabbable = this.colisioActual.gameObject.GetComponent<OVRGrabbable>();
        ...
        if (closestGrabbable != null)
        {
            ...
            for (int j = 0; j < closestGrabbable.grabPoints.Length; ++j)
            {
                Collider grabbableCollider = closestGrabbable.grabPoints[j];
                // Store the closest grabbable
                Vector3 closestPointOnBounds = grabbableCollider.ClosestPointOnBounds(m_gripTransform.position);
                float grabbableMagSq = (m_gripTransform.position - closestPointOnBounds).sqrMagnitude;
                if (grabbableMagSq < closestMagSq)
